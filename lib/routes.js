...
module.exports =
		function(config) {
			...
			routes.push({
				...
				method : 'PUT',
				config : {
					handler : handlers.updateLogManagerSettings,
					description : 'Updates an existing LogManager\'s settings',
					validate : {
						payload : {
							logDir : types.String().required().regex(/^\/.+/),
							logLevel : types.String(),
			...
			routes.push({
				...
				config : {
					...
					description : 'Returns the list of files within the log dir. The log file naming pattern is [event].[pid].log.[seq#](.gz)',
					validate : {
						path : {
							logDir : types.String().required().regex(/^\/.+/)
						}
			...
			routes.push({
				path : path('/logManager/deleteAllNonActiveLogFiles/{logDir*}'),
				method : 'POST',
				config : {
					handler : handlers.deleteAllNonActiveLogFiles,
					description : 'Deletes all non-active log files in the log dir. '
							+ 'A non-active log file is defined as having no live processes matching he PID in the log file name.',
					validate : {
						path : {
							logDir : types.String().required().regex(/^\/.+/)
						}
